...
def run_all_compatibility_checks():
    failures = []
    ...
    _FEATURES_REQUIRED = r1.features_required
    ...
    _EXPECTED_PARTIAL = len(partial_row) - 1
...
def test_compatibility_suite():
    failures = run_all_compatibility_checks()
    assert len(failures) == 0, f"{len(failures)} test(s) failed in compatibility suite: {failures}"
